...
def run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step_number, config):
    ...
    adam_config = {key:val for key,val in config.items() if key in ('beta1', 'beta2', 'epsilon')}
    optimizer_state = apply_adam_step_to_all_parameters(parameter_list, optimizer_state, lr, **adam_config)
...
def run_training_loop_for_steps(batches, parameter_list, model_params, optimizer_state, num_steps, config):
    ...
    for step in range(1,num_steps+1):
        ind = (step-1)%len(batches)
        src_batch, tgt_batch = batches[ind]

        loss = run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step, config)
        losses.append(loss)
...
def compute_candidate_scores(beam_scores, next_token_log_probs):
    ...
    return next_token_log_probs + beam_scores[:, None]
...
def select_top_k_candidates(candidate_scores, k):
    ...
    scores, top_k_inds = torch.topk(candidate_scores.flatten(), k=k)
    spatial_inds = torch.stack(torch.unravel_index(top_k_inds, candidate_scores.shape), dim=1)
    return dict(
        beam_indices=spatial_inds[:,0],
        token_ids=spatial_inds[:,1],
        scores=scores
    )
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    return torch.cat((beam_sequences[beam_indices], token_ids.unsqueeze(1)), dim=1)
...
def mark_finished_beams(token_ids, finished_flags, end_token_id):
    ...
    return torch.where(~finished_flags, token_ids==end_token_id, finished_flags)
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    penalized_scores = [score/compute_length_penalty(len(seq),alpha) for seq, score in zip(finished_sequences, finished_scores)]
    best_seq_id = max(range(len(finished_sequences)), key=lambda x: penalized_scores[x])
    return dict(
        sequence=finished_sequences[best_seq_id],
        score=penalized_scores[best_seq_id]
    )
